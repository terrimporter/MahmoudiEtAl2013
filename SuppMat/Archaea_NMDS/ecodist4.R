...
pdf(file="NBC_NMDS_order_NG.pdf")
par(mfrow=c(2,2))

a<-read.csv("NBC_order_NG.csv",head=TRUE,row.names=1)
a
b<-as.matrix(a)
b
library(Matrix)
...
pdf(file="NBC_NMDS_order_CS.pdf")
par(mfrow=c(2,2))

A<-read.csv("NBC_order_CS.csv",header=TRUE,row.names=1)
A
B<-as.matrix(A)
B
library(Matrix)
...
save.image(file="NBC_NMDS_order.RData")
